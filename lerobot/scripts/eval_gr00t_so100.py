...
class Gr00tRobotInferenceClient:
    ...
    def get_action(self, imgs, state):
        ...
        obs_dict["state.single_arm"] = state[:5][np.newaxis, :].astype(np.float64)
        obs_dict["state.gripper"] = state[5:6][np.newaxis, :].astype(np.float64)
    ...
    def sample_action(self):
        obs_dict = {
            ...
            "video.frontal": np.zeros((1, self.img_size[0], self.img_size[1], 3), dtype=np.uint8),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--lang_instruction", type=str, default="Grasp a lego block and put it in the bin."
    )
